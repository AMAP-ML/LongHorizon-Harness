...
Mirrors the core openclaw `__computer__` plugin
(`weavebench/assets/computer_tool_plugin/index.ts`) behavior, with one
additional harness evidence action:

  * Same 8 actions: click, double_click, drag, keypress, move, scroll,
    type, wait — plus screenshot / cursor_position no-ops and the
...
RESULTS_DIR = os.environ.get(
    "WEAVEBENCH_MCP_RESULTS_DIR", "/tmp_workspace/results"
)
RETURN_SCREENSHOT_MAX_WIDTH = int(
    os.environ.get("WEAVEBENCH_MCP_RETURN_SCREENSHOT_MAX_WIDTH", "0")
)
RETURN_SCREENSHOT_JPEG_QUALITY = max(
    25,
    min(95, int(os.environ.get("WEAVEBENCH_MCP_RETURN_SCREENSHOT_JPEG_QUALITY", "60"))),
)
...
# Coordinate mode: "norm1000" scales model outputs from 0-1000 -> screen pixels;
# "auto" scales only when both x,y are in [0,1000]; "pixel" passes through raw.
COORD_MODE = os.environ.get("WEAVEBENCH_COMPUTER_COORD_MODE", "pixel").lower()
sys.stderr.write(f"[weavebench-mcp] COORD_MODE={COORD_MODE} (env={os.environ.get('WEAVEBENCH_COMPUTER_COORD_MODE', '<unset>')})\n")
OBSERVE_ONLY = os.environ.get("WEAVEBENCH_COMPUTER_OBSERVE_ONLY", "").lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def save_current_screenshot(action: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        meta = {
            ...
            "path": str(target),
            "producer": "mcp__weavebench_computer__computer",
...
def capture_screenshot(action_label: str) -> tuple[str, str] | None:
    ...
    try:
        ...
        # WEAVEBENCH_MCP_RETURN_SCREENSHOT_MAX_WIDTH>0 only for explicit
...
def execute_action(action: dict) -> dict:
    ...
    if OBSERVE_ONLY and t not in ("screenshot", "cursor_position", "wait"):
        return {
            "ok": False,
            "err": (
...
TOOL_DESCRIPTION = "\n".join([
    ...
    "Coordinate mode follows WEAVEBENCH_COMPUTER_COORD_MODE. In norm1000 mode, x/y coordinates are normalized to a 0-1000 screenshot space and are scaled to the current screen. If you deliberately need raw pixels for one action, set coordinate_space:\"pixel\" on that action.",
    "Call with either:",

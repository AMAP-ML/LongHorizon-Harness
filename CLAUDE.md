...
| File | n | Covers |
|---|---|---|
| `test_provider_config.py` | 51 | Precedence chain, `require()`, ancestor/package root searches, flat-schema validation (gptme requires `providers`, claude/codex/opencode reject `providers`/`provider`, unknown top-level keys raise) |
